    ...
    while True:
        ...
        if choice == '1':
            ...
            if not books:
                ...
                for b in books:
                    # Explicitly convert to standard Python values to guarantee terminal rendering
                    print(f"ID: {int(b['book_id'])} | '{str(b['title'])}' by {str(b['author'])} | ISBN: {str(b['isbn'])} | Status: {str(b['status'])}")
                
        elif choice == '3':
            print("\n--- Search Inventory ---")
            query = input("Enter title keyword to search: ")
            books = search_book_by_title(query)
            if not books:
                print("No matching books found.")
            else:
                for b in books:
                    print(f"ID: {int(b['book_id'])} | '{str(b['title'])}' by {str(b['author'])} | Status: {str(b['status'])}")
            ...
            if not members:
                ...
                for m in members:
                    print(f"ID: {int(m['member_id'])} | Name: {str(m['name'])} | Email: {str(m['email'])} | Joined: {str(m['join_date'])}")
            ...
            print("❌ Invalid selection. Please choose an option between 0 and 8.")
